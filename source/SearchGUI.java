	...
	public SearchGUI() {	
		...
		total.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
                ...
                Object[][] data = Search.getAllInstitution();
                for (Object[] row : data) {
		...
		typeList.addListSelectionListener(e -> {
			if (!e.getValueIsAdjusting()) {
				...
				if (!selected.equals("(select)") && selected != null) {
					tableModel.setRowCount(0);
	                Object[][] data = Search.getInstitutionByType(selected);
	                for (Object[] row : data) {
	                    tableModel.addRow(row);
	                }
				} 
		...
		districtList.addListSelectionListener(e -> {
			if (!e.getValueIsAdjusting()) {
				...
				if (!selected.equals("(select)") && selected != null) {
					tableModel.setRowCount(0);
					Object[][] data = Search.getInstitutionByDistrict(selected);
	                for (Object[] row : data) {
	                    tableModel.addRow(row);
	                }
				} 
		...
		rating.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				Object[][] data = Search.getInstitutionByRating();
                for (Object[] row : data) {
                    tableModel.addRow(row);
                }
			}
